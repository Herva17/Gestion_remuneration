...
<div class="container">
    ...
    <div class="top">
        <div>
            ...
            <p>Gérez les services de l'établissement</p>
        </div>
        <a href="add.php" class="btn btn-success"><i class="fas fa-plus"></i> Ajouter un Service</a>
    </div>
    ...
    <div class="grid-4">
        <div class="card border-blue">
            <div class="number text-blue"><?php echo $totalServices; ?></div>
            <div class="label"><i class="fas fa-cogs"></i> Total Services</div>
        </div>
        <div class="card border-green">
            <div class="number text-green"><?php echo $totalServices > 0 ? $totalServices : 0; ?></div>
            <div class="label"><i class="fas fa-check-circle"></i> Actifs</div>
        </div>
        <div class="card border-purple">
            <div class="number text-purple"><?php 
                $totalAgents = 0;
                foreach ($services as $service) {
                    $totalAgents += count($service->getAgents());
                }
                echo $totalAgents;
            ?></div>
            <div class="label"><i class="fas fa-users"></i> Agents affectés</div>
        </div>
        <div class="card border-orange">
            <div class="number text-orange"><?php echo count($services) > 0 ? round($totalAgents / count($services), 1) : 0; ?></div>
            <div class="label"><i class="fas fa-chart-bar"></i> Moy. par service</div>
        </div>
    </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <?php endif; ?>
        <span class="text-sm text-gray-500"><?php echo $totalServices; ?> service<?php echo $totalServices > 1 ? 's' : ''; ?></span>
    </div>
    ...
    <div class="card">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($services as $service): 
                            $nbAgents = count($service->getAgents());
                        ?>
                        <tr>
                            ...
                            <td>
                                <div style="display:flex;align-items:center;gap:8px;">
                                    ...
                                    <strong><?php echo htmlspecialchars($service->getDesignation()); ?></strong>
                                </div>
                            </td>
                            <td><?php echo htmlspecialchars($service->getDescription() ?: '-'); ?></td>
                            <td>
                                <span class="badge" style="background:#dbeafe;color:#2563eb;padding:2px 10px;border-radius:12px;font-size:12px;">
                                    <?php echo $nbAgents; ?> agent<?php echo $nbAgents > 1 ? 's' : ''; ?>
                                </span>
                            </td>
                            <td>
                                <div style="display:flex;gap:6px;flex-wrap:wrap;">
                                    <a href="view.php?id=<?php echo $service->getId(); ?>" 
                                       style="color:#2563eb;text-decoration:none;" title="Voir">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="edit.php?id=<?php echo $service->getId(); ?>" 
                                       style="color:#ca8a04;text-decoration:none;" title="Modifier">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="delete.php?id=<?php echo $service->getId(); ?>" 
        ...
        <div class="mt-4 flex justify-between items-center text-sm text-gray-500">
            <span>Total : <strong><?php echo $totalServices; ?></strong> service<?php echo $totalServices > 1 ? 's' : ''; ?></span>
